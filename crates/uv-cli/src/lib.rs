...
pub struct PspfPackageArgs {
    /// Path to the `uv` binary to use as the base of the PSPF package.
    /// This `uv` binary (with an embedded public key) will become the executable.
    /// If not provided, the currently running `uv` executable is used.
    #[arg(long)]
    pub uv_binary_to_package: Option<PathBuf>,
    ...
    /// The corresponding public key will be embedded into the `uv` binary portion of the PSPF.
    #[arg(long, required = true)]
